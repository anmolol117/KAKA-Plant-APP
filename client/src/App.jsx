...
export default function App() {
  ...
  const ensureMediaPlayback = () => {
    ...
    if (effectiveMood === "sleep" && snoreRef.current) {
      snoreRef.current.play().catch(() => {});
    }
  };
        ...
        {loading ? (
          ...
          <main className="space-y-6">
            <div className="relative">
              <div className="absolute right-3 top-3 z-30 rounded-[1.25rem] border border-white/10 bg-slate-950/45 px-3 py-2.5 text-white shadow-card backdrop-blur-xl sm:right-4 sm:top-4 sm:rounded-[1.5rem] sm:px-5 sm:py-4">
                <p className="text-[10px] uppercase tracking-[0.18em] text-white/70 sm:text-xs sm:tracking-[0.2em]">Current mood</p>
                <p className="font-display text-xl capitalize sm:text-3xl">{effectiveMood || "sleep"}</p>
              </div>
              <PlantCharacter
                mood={effectiveMood}
